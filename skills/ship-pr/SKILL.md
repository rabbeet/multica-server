---
name: ship-pr
description: Use after agent finishes implementation in worktree — opens a PR via `gh pr create`, links the plan committed in /publish-plan, sets assignee, ensures the PR description points reviewers at the audit log entry. End-of-task wrapper for /plan-and-implement Phase 3.
---

# /ship-pr

Wrapper around `gh pr create` for agent-host worker flow. Closes the loop on `/plan-and-implement`.

## When to invoke

Phase 3 of `/plan-and-implement`. You have:
- Committed your code on branch `agent-N/<task-slug>` and pushed.
...
If current branch is `main`, `master`, or anything not under `agent-N/*` / `feature/*`, refuse to ship. The agent should never reach this skill from main — `agent-spawn.sh` always creates a per-task branch. If you somehow end up on main, STOP and ask the user what happened.
...
- [ ] {test 1 from plan}
- [ ] {test 2 from plan}
- [ ] (if migration touched) Verified `php artisan migrate` against dev-PG sidecar
- [ ] (if migration touched) Verified `php artisan migrate:rollback` cleanly reverses

## Agent context

Built by agent-{N} in agent-host container. Plan + implementation in one session.
...
- It does NOT push to `main` (denied — branch protection on receiving repo).
